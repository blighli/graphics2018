...
Camera mycamera(glm::vec3(0.0f, 10.0f, 10.0f));
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "LinBingru", NULL, NULL);
	if (window == NULL)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 earthModel;
		glm::vec3 earthPosition = glm::vec3(5.0*sin(-0.2*programTime-1), 0.0f, 3.0*cos(-0.2*programTime-1) );		
		...
		sunModel = glm::rotate(sunModel, (float)(programTime*0.1), glm::vec3(0.0f, 1.0f, 0.0f));
