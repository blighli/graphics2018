...
void Model::init() {
	...
	m_ibo = new IndexBuffer(&m_Indicies[0].x, m_Indicies.size() * 3);

	m_vao->addBuffer(new Buffer(&m_Verticies[0].x, m_Verticies.size() * 3, 3), 0);
	m_vao->addBuffer(new Buffer(&m_Normals[0].x, m_Normals.size() * 3, 3), 1);
	//m_vao->addBuffer(new Buffer(&m_Textures[0].x, m_Textures.size() * 2, 2), 2);
}
