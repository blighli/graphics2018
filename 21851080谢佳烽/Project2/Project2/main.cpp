...
float epsilon = (float)(PI * 10 / 180);    
float mmax = (float)(PI / 2 - epsilon);
float mmaxvalue = (float)(log(tan(mmax) + 1.0 / cos(mmax)));
float ts = (float)((PI / 2 - epsilon) / (PI / 2));

void getSTTheta(float theta, float fai, float &s, float &t)
{
	s = (float)(0.5 - (fai) / (2 * PI));
	float mtheta = (float)(-(theta - PI / 2) * ts);
	t = (float)(log(tan(mtheta) + 1.0 / cos(mtheta)));
	t = (float)(0.5 + 0.5 * t / mmaxvalue);
}
...
int main()
{
	...
	unsigned int diffuseMap1 = loadTexture("earth.jpg");
	unsigned int diffuseMap2 = loadTexture("moon.jpg");
	unsigned int diffuseMap3 = loadTexture("sun.jpg");
