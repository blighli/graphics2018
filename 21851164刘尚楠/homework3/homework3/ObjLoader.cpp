...
//  ObjLoader.cpp
//
//  Created by Marveliu on 2019/1/3.
//  Copyright © 2019 Marveliu. All rights reserved.
//
...
void ObjLoader::Draw(){
    ...
    for (int i = 0; i < fSets.size(); i++) {
        ...
        if ((fSets[i]).size() != 3) {
            ...
            SV1[0] = (vSets[firstVertexIndex-1])[0];//第一个顶点
            SV1[1] = (vSets[firstVertexIndex-1])[1];
            SV1[2] = (vSets[firstVertexIndex-1])[2];
            
            SV2[0] = (vSets[secondVertexIndex-1])[0]; //第二个顶点
            SV2[1] = (vSets[secondVertexIndex-1])[1];
            SV2[2] = (vSets[secondVertexIndex-1])[2];
            
            SV3[0] = (vSets[thirdVertexIndex-1])[0]; //第三个顶点
            SV3[1] = (vSets[thirdVertexIndex-1])[1];
            SV3[2] = (vSets[thirdVertexIndex-1])[2];
